...
class UIComponentFactory:
    ...
    def create_package_type_frame(parent: tk.Widget, variable: tk.StringVar,
                                 command: Optional[Callable[[], None]] = None) -> ttk.LabelFrame:
        """创建包类型选择框架"""
        ...
        for i, (pkg_type, display_name) in enumerate(AppConstants.PACKAGE_TYPE_NAMES.items()):
            ...
            radio = ttk.Radiobutton(frame, **radio_kwargs)
    ...
    def create_folder_selection_frame(parent: tk.Widget, variable: tk.StringVar,
                                     select_command: Callable, preview_command: Optional[Callable] = None) -> Dict[str, Any]:
        """创建文件夹选择框架"""
...
class WindowFactory:
    ...
    def create_dialog_window(parent: tk.Widget, title: str, geometry: str,
        ...
        if hasattr(parent, 'winfo_toplevel'):
            window.transient(cast(tk.Tk, parent.winfo_toplevel()))
        else:
            window.transient(cast(tk.Tk, parent))
